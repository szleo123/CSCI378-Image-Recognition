...
# Override method: _updateLocation
    ...
    def _updateLocation(self):
        ...
        """
#// Check this code
        baseline = self.y + self.getHeight() - self.getAscent()
        gw = self.getWindow()
        if gw is None: return
        tkc = gw.canvas
        coords = tkc.coords(self.tkid)
...
# Override method: _install
    ...
    def _install(self, target, ctm):
        ...
        self.tkid = tkc.create_polygon(*coords)
        self.updateColor()
...
# Constructor: GPoint
    ...
    def __init__(self, x=0.0, y=0.0):
        """
        Initializes a <code>GPoint</code> object with the specified
...
class GDimension:
    """
    This class contains real-valued witdth and height fields.  It is used
...
class _EventManager:
    ...
    def addEventListener(self, type, fn):
        ...
        if type == "click":
            ...
            if self.releaseHandler is None:
                self.releaseHandler = self.releaseAction
                tkc.bind("<ButtonPress-1>", self.releaseHandler)
